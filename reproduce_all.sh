...
for s in "${SWEEP_SEEDS[@]}"; do
  ...
  j $PY secondflow traincell --variant wrsg_gate  --seed $s
  j $PY secondflow traincell --variant wrsg        --seed $s --profile full --scale 1.0
  j $PY secondflow traincell --variant wrsg        --seed $s --profile full --scale 0.25
  j $PY secondflow traincell --variant wrsg        --seed $s --profile full --scale 0.5
  j $PY secondflow traincell --variant wrsg        --seed $s --profile spectral_only
  j $PY secondflow traincell --variant wrsg        --seed $s --profile spectral_lowk
done
for s in 101 149; do j $PY secondflow traincell --variant wrsg --seed $s --profile spectral_only; done
...
for v in vanilla wrsg_gate wrsg; do for s in "${SWEEP_SEEDS[@]}"; do j $PY thirdflow traincell --variant $v --seed $s; done; done
j $PY thirdflow aggregate

echo "### 64^2 resolution transfer  -> res64_poc_metrics.csv"
j $PY resolution gendata
for v in vanilla wrsg_gate wrsg; do for s in "${SWEEP_SEEDS[@]}"; do j $PY resolution traincell --variant $v --seed $s; done; done
