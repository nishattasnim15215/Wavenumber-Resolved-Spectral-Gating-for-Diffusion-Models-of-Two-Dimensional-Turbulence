...
class TurbulenceDenoiser(nn.Module):
    ...
    selected per ablation variant: vanilla / se / fno (FNO bottleneck) / wrsd_gate (gate only) /
    wrsd_phys (losses only) / wrsd (main model: gate + losses) / wrsd_fno (gate + FNO + losses)."""
    _BOTTLENECK_OF = {
        "vanilla":   "none",
        "se":        "se",
        "fno":       "fno",
    ...
    def __init__(self, variant, base_ch=48, emb_dim=128, fno_modes=8,
        ...
        elif self._arch == "wrsd": self.bottleneck = WavenumberResolvedGate(bch, emb_dim, gate_bins, gate_rank)
        elif self._arch == "wrsd_fno": self.bottleneck = WRSGFNOBottleneck(bch, emb_dim, gate_bins, gate_rank, fno_modes)
    ...
    def forward(self, x, c_noise, nu_cond):
        ...
        if self._arch in ("se", "wrsd", "wrsd_fno"):
...
VARIANT_ORDER = ["vanilla", "se", "fno", "fno_operator",
                 "wrsd_gate", "wrsd_phys", "wrsd", "wrsd_fno"]
...
FLUX_SHOW_VARIANTS = ["vanilla", "fno", "wrsd", "wrsd_fno"]
...
def run_gate_sensitivity(cfg, train_pool, test_set, device):
    ...
    evaluating wrsd per configuration; writes a CSV of LSD mean/std vs config."""
    ...
    print(f"[Sweep] gate sensitivity over {len(configs)} configs x "
          f"{len(cfg.SWEEP_SEEDS)} seeds (variant=wrsd).")
    for (bins, rank) in configs:
        ...
        for seed in cfg.SEEDS:
            model, sched, _, _ = train_one_model(
                "wrsd", seed, cfg, train_pool, device,
                use_physics=True, use_flux=True)
            agg = evaluate(model, sched, test_set, cfg, device, "wrsd", seed)[0]
...
PHYSICS_VARIANTS = {"wrsd_phys", "wrsd", "wrsd_fno"}
FLUX_VARIANTS    = {"wrsd", "wrsd_fno"}
...
_VARIANT_COST_RANK = {"wrsd_fno": 0, "fno_operator": 1, "wrsd": 2, "wrsd_gate": 3,
                      "fno": 4, "wrsd_phys": 5, "se": 6, "vanilla": 7}
...
def finalize_outputs(a, cfg):
    ...
    for target in [t for t in ["wrsd", "wrsd_fno"] if t in present]:
...
def get_gate(model):
    """Return the WavenumberResolvedGate inside a wrsd / wrsd_fno denoiser (or None)."""
...
OOD_VARIANTS = ["vanilla", "fno", "wrsd", "wrsd_fno"]
...
def run_step_ablation(cfg, device, variants=("vanilla", "wrsd", "wrsd_fno"),
...
def run_mechanism(cfg, device, variant="wrsd", seeds=None, n_sigma=40):
...
def run_rootcause(cfg, device, variants=("vanilla", "wrsd"), seed=None,
...
def run_gradflow(cfg, device, seeds=None, n_sigma=24, n_fields=64):
    ...
    gate (wrsd) and without it (wrsd_phys). Reports the gradient norm reaching the deepest
    encoder feature vs sigma for both, and the gate's share of the parameter gradient for
    wrsd. Writes gradflow.csv and gradflow.png."""
    seeds = seeds or cfg.SEEDS
    payload = load_payload(cfg)
    fields = payload["fields"]; re = payload["re_labels"]
    full = TurbulenceDataset(fields, re)
...
@torch.no_grad()
def run_uq_depth(cfg, device, variant="wrsd", seeds=None, n_cond=20, n_ens=None):
...
def run_sweep_cell(cfg, device, train_pool, test_set, bins, rank, seed):
    ...
    cfg.CKPT_DIR.mkdir(parents=True, exist_ok=True)
    model, sched, _, _ = train_one_model("wrsd", seed, cfg, train_pool, device,
                                            use_physics=True, use_flux=True)
    agg = evaluate(model, sched, test_set, cfg, device, "wrsd", seed)[0]
...
def run_losscell(cfg, device, train_pool, test_set, tag, seed,
    ...
    model, sched, _, _ = train_one_model("wrsd", seed, cfg, train_pool, device,
                                            use_physics=True, use_flux=True)
    agg, _, per_regime, _, _, _ = evaluate(model, sched, test_set, cfg, device, "wrsd", seed)
...
def aggregate_ablations(cfg):
    ...
    base = summ[(summ.variant == "wrsd") & (summ.seed.isin(SWEEP_SEEDS))]
...
def _reviews_main():
    ...
    if args.cmd == "ood":
        ...
        run_step_ablation(Config, device,
                          ...
                          ("vanilla", "wrsd", "wrsd_fno"), seeds=args.seeds)
    elif args.cmd == "speed":
        ...
        run_rootcause(Config, device,
                      variants=tuple(args.variants) if args.variants else ("vanilla", "wrsd"))
...
def run_smoke(cfg, device):
    ...
    run_rootcause(cfg, device, variants=("vanilla", "wrsd"), n_sigma=6, n_fields=16)
    print("[Smoke] speed ...")
    run_speed(cfg, device, variants=["vanilla", "wrsd"], n_steps=10, n_fields=8, reps=1)
    print("[Smoke] steps ...")
    run_step_ablation(cfg, device, variants=("wrsd",), seeds=[cfg.SEEDS[0]],
                      step_grid=[10, 20], n_eval=24)
    print("[Smoke] ood (1 nu, 1 variant) ...")
    global OOD_INTERP, OOD_EXTRAP
    OOD_INTERP, OOD_EXTRAP = [0.0085], []
    run_ood(cfg, device, variants=["wrsd"], seeds=[cfg.SEEDS[0]], n_snap=24, n_steps=10)
    print("[Smoke] done.")
...
# ---------------------------------------------------------------------------
# Scalar config (reuses the wrsd training loop unchanged)
# ---------------------------------------------------------------------------
...
SCALAR_VARIANTS = ["vanilla", "wrsd_gate", "wrsd"]
...
def run_scalar_cell(variant, seed, device, epochs=None, lam_scale=1.0, profile="full"):
    ...
    use_phys = variant in ("wrsd_phys", "wrsd", "wrsd_fno")
...
def aggregate_scalar_retune():
    """Collect the scalar retune/profile cells against the wrsd_gate (no losses) and wrsd
...
def _secondflow_main():
    ...
    p.add_argument("cmd", choices=["validate", "gendata", "traincell", "aggregate",
                                   "retuneagg"])
    p.add_argument("--variant", default="wrsd")
    p.add_argument("--seed", type=int, default=29)
    p.add_argument("--scale", type=float, default=1.0)
...
CELL_FORCING_K = 6
CELL_SNAPSHOTS = 800
CELL_VARIANTS = ["vanilla", "wrsd_gate", "wrsd"]
...
def run_cell(variant, seed, device, epochs=None):
    ...
    use_phys = variant in ("wrsd_phys", "wrsd", "wrsd_fno")
    use_flux = variant in ("wrsd", "wrsd_fno")
    model, sched, _, compute = train_one_model(variant, seed, cfg, train_pool, device,
...
def run_downstream(cfg, device, variants=("wrsd", "vanilla"), seed=None,
